...
`.planning/PROJECT.md` holds the scope and the decisions.
`.planning/research/` holds what the surveys found.
Read them before you plan work.

## The gate

Every one of these, before every commit.
Not a selection.
...
`build` does not compile a `#[cfg(test)]` module.
A broken test helper passes `build` and fails later under clippy.

## The file is hostile

Every input is untrusted.
A person who uses this tool has a file that is already damaged, or a file that
somebody made to break a parser.

- **No panic on any input. Ever.**
- Do not index a slice with `[]`.
  Use `get` and handle the `None`.
- Do not call `unwrap` or `expect` on a value that comes from the file.
- Do not size an allocation from a length field in the file.
  Check that length against the real size of the file first.
- Do not add two offsets that come from the file with `+`.
  Use `checked_add`.
  A `u32` offset plus a `u32` length overflows, and the sum then points inside
  the file.
- An error names the byte offset and what the code expected to find there.

Fuzzing is part of the gate, not an extra.
A crash that the fuzzer finds becomes a test case in the repository.

## What may enter this repository

- A program enters `corpus/` only if its licence permits redistribution.
  `corpus/NOTICES` names the origin, the licence, and the upstream commit of
  each one.
- Everything else is fetched at run time from a manifest of pinned hashes.
  Commit the manifest.
  Never commit the bytes.
- No binary, no source file, and no derived fixture from a third party system
  that the author does not own may enter this repository.
  This includes a fixture that was calculated from such a file.

## Measurement

The tool reports a recovery ratio: how many items it recovered against how many
the original source holds.

- Measure it against the original source that the executable was built from.
  Do not measure it against what the tool itself produced.
- A test pins the ratio for each corpus program.
  When the ratio moves, the failure message gives the new number.
- Give the number that you can prove.
  A commercial tool in this field advertises "85% recovery" with no evidence
  behind it.
  Do not copy that habit.

## What a test may hold on to

- A test asks the binary a question.
  It compares the answer against the original source file that is committed
  next to the binary.
- A round trip through our own writer and our own parser is not verification.
  It proves only that the code agrees with itself.
  Keep it as a unit test.
  Never count it as a check of correctness.
- When you add a test, break the thing it covers on purpose and watch it fail.
  A test that cannot fail is worse than no test, because it makes something
  look guarded while it drifts.

## Prior art

Read the other tools for facts.
Do not copy their code.

Semi VB Decompiler is the most useful of them and it has no licence file, so
all rights in it are reserved.
Read it to learn what a structure holds.
