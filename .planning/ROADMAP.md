...
**Plans**: 18/18 plans executed. Plans 03-11 to 03-18 close the gaps the
...
- [x] 03-07-PLAN.md - `.frx` blob extraction from the inline property stream, with image format detection
...
- [x] 03-14-PLAN.md - The two-level menu close rule, the defensive tail bound replaced, and code review finding WR-02
...
- [x] 03-18-PLAN.md - The native handler address wired from `decode_stub` to `EventReport` and to the command line, with an end to end test, and the FRM-03 and FRM-06 checkboxes set by measurement
...
- [x] 04-01-PLAN.md - The end to end tracer, the `write` module set, `model.rs`, `SafeName`, the 40-byte clamp, the identifier check, and the Windows-1252 and CRLF emitter primitives
...
- [x] 04-06-PLAN.md - `report.rs`: the flat item array, the path key, the three confidence words, `basis` and `evidence`, the defect array, deterministic output

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 04-04-PLAN.md - `write/frm.rs`: the `.frm` and the `.frx` as one component driven by one cursor. Not splittable
- [x] 04-07-PLAN.md - The uncertainty comment emitter, in code regions only, never in a `Begin` block and never in the `.vbp`

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 04-08-PLAN.md - The `extract` subcommand: `-o`, `--report`, `--force`, the resolved output directory, the containment check, exit codes
...
- [x] 05-06-PLAN.md - The stable fuzz smoke test, a fixed seed mutation of corpus files, in the normal gate, no dependency
...
- [x] 06-03: The public library API review, the doc comments, and a clean `cargo doc --no-deps`
- [x] 06-04: The version, the tag check, and the `.gitattributes` audit for `*.frx` and `*.ctx`
