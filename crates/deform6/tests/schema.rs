...
/// A report that carries a `RunsPastEnd` defect validates, and the schema
/// refuses the same defect when one of its three fields is missing.
///
/// No corpus program raises this kind, for the reason the tests above give
/// for their own kinds.
#[test]
fn a_report_that_carries_a_runs_past_end_defect_validates() {
    let validator = compiled_schema();
    let exe = corpus_root().join("vb6-code/Fire-effect/Fast_Flames.exe");
    let mut value = report_value_for(&exe);
    let defect = Defect {
    ...
    assert!(
        errors.is_empty(),
        "a report with a RunsPastEnd defect failed schema validation:\n{}",
        errors.join("\n")
    );

    for field in ["offset", "len", "end"] {
        let mut missing = value.clone();
        missing["defects"][at]["kind"]["RunsPastEnd"]
            .as_object_mut()
            .expect("the kind must be an object")
            .remove(field);
        assert!(
...
/// A report that carries a `UnknownValue` defect validates, and the schema refuses
/// the same defect when one of its fields is missing.
///
/// No corpus program raises this kind, for the reason the tests above give
/// for their own kinds.
#[test]
fn a_report_that_carries_an_unknown_value_defect_validates() {
    let validator = compiled_schema();
    let exe = corpus_root().join("vb6-code/Fire-effect/Fast_Flames.exe");
    let mut value = report_value_for(&exe);
    let defect = Defect {
    ...
    assert!(
        errors.is_empty(),
        "a report with a UnknownValue defect failed schema validation:\n{}",
        errors.join("\n")
    );

    for field in ["offset", "value"] {
        let mut missing = value.clone();
        missing["defects"][at]["kind"]["UnknownValue"]
            .as_object_mut()
            .expect("the kind must be an object")
            .remove(field);
        assert!(
...
/// A report that carries a `ItemTypeUnknown` defect validates, and the schema refuses
/// the same defect when one of its fields is missing.
///
/// No corpus program raises this kind, for the reason the tests above give
/// for their own kinds.
#[test]
fn a_report_that_carries_an_item_type_unknown_defect_validates() {
    let validator = compiled_schema();
    let exe = corpus_root().join("vb6-code/Fire-effect/Fast_Flames.exe");
    let mut value = report_value_for(&exe);
    let defect = Defect {
    ...
    assert!(
        errors.is_empty(),
        "a report with a ItemTypeUnknown defect failed schema validation:\n{}",
        errors.join("\n")
    );

    for field in ["offset", "value"] {
        let mut missing = value.clone();
        missing["defects"][at]["kind"]["ItemTypeUnknown"]
            .as_object_mut()
            .expect("the kind must be an object")
            .remove(field);
        assert!(
