...
fn an_empty_file_is_not_a_portable_executable() {
    assert_eq!(
        deform6::inspect(&[], &builtin_table(), deform6::journal::Mode::Strict),
        Err(Refusal::NotPe)
    );
}
...
fn a_short_text_file_is_not_a_portable_executable() {
    ...
    assert_eq!(
        deform6::inspect(data, &builtin_table(), deform6::journal::Mode::Strict),
        Err(Refusal::NotPe)
    );
}
...
fn a_pe_file_importing_no_recognised_visual_basic_runtime_is_refused() {
    ...
    assert_eq!(
        deform6::inspect(&bytes, &builtin_table(), deform6::journal::Mode::Strict),
...
fn a_truncated_visual_basic_6_executable_is_damaged_and_not_refused_as_a_non_pe() {
    ...
    let refusal =
        deform6::inspect(half, &builtin_table(), deform6::journal::Mode::Strict).unwrap_err();
    assert!(
...
fn the_unmodified_corpus_file_is_accepted() {
    let data = mandelbrot();
    let report = deform6::inspect(&data, &builtin_table(), deform6::journal::Mode::Strict);
    assert!(
        report.is_ok(),
